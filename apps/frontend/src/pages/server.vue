<template>
    ...
    <div>
        ...
        <div class="game-page container" v-if="cf && cf.items && cf.items.length > 0">
            ...
            <div style="justify-content: center; align-items: center">
                ...
                <h2 data-v-56edd70f=""
                    ...
                    已支持100+ <span data-v-56edd70f="" class="font-bold"> 主流整合包</span> ， 快速一键部署，无需繁琐上传和配置即可联机。
                    <br />
                    <br />
                    与 <span data-v-56edd70f="" class="font-bold"> {{ cf.name }}</span>
                    联动合作，您的购买将会把销售收益的20%-30%的归于{{ cf.name }}所有用于创作持续性收益
                </h2>
                <h1 style="font-size: 30px; font-weight: bold; color: #e5e5e5;margin-top: 30px;">咨询客服</h1>
                <h2 data-v-56edd70f=""
                    class="relative m-0  text-base font-normal leading-[155%] text-secondary md:text-[18px]">
                ...
                <h1 style="font-size: 30px; font-weight: bold; color: #e5e5e5;margin-top: 30px;">选择合适您的套餐</h1>
                <p class="text-[15px]">所标注人数为推荐同时在线范围，请根据实际情况选择合适的套餐</p>
                <p class="text-[15px]">游戏人数在套餐推荐人数之间请参照购买预算选择套餐 </p>
                <p class="text-[15px]">例如：我想玩乌托邦，我有五个人玩，我预算足够我购买高频88或者发烧108，我预算不够我购买高频58或发烧78 </p>
                <h1 style="font-size: 30px; font-weight: bold; color: #e5e5e5;margin-top: 30px;">优惠赠送</h1>
                <p class="text-[15px]">下单赠送7天时长,带游戏内3张风景图好评再送7天</p>

                <!-- 套餐列表 ，横着3个card -->
                <div class="flex flex-row gap-4 resource-list">


                    <div v-for="item in cf.items" :key="item.name" class="card w-full min-h-200 flex flex-col">
                        <h2>{{ item.name }}</h2>
