<template>
  <section class="experimental-styles-within overflow-visible">
    <div v-if="currentMember && isPermission(currentMember?.permissions, 1 << 0)" class="card flex items-center gap-4">
      <FileInput :max-size="1073741824" :accept="acceptFileFromProjectType(project.project_type)" prompt="上传版本(文件)"
        class="btn btn-primary" aria-label="上传版本" @change="handleFiles">
    ...
    <div class="mb-3 flex flex-wrap gap-2">
      <VersionFilterControl ref="versionFilters" :versions="props.versions" @switch-page="switchPage" />
      <Pagination :page="currentPage" class="ml-auto mt-auto" :count="Math.ceil(filteredVersions.length / 20)"
        :link-function="(page) => `?page=${currentPage}`" @switch-page="switchPage" />
    </div>
    <div v-if="versions.length > 0"
      ...
      <div class="versions-grid-row">
        ...
        <div class="text-sm font-bold text-contrast max-sm:hidden sm:max-xl:collapse sm:max-xl:hidden">
          游戏版本
        </div>
        <div class="text-sm font-bold text-contrast max-sm:hidden sm:max-xl:collapse sm:max-xl:hidden">
          平台
      ...
      <template v-for="(version, index) in filteredVersions.slice((currentPage - 1) * 20, currentPage * 20)"
        :key="index">
        <div :class="`versions-grid-row h-px w-full bg-button-bg ${index === 0 ? `max-sm:!hidden` : ``}`"></div>
        <div class="versions-grid-row group relative">
          <nuxt-link
            ...
            :to="`/${project.project_type}/${project.slug ? project.slug : project.id
              }/version/${encodeURI(version.displayUrlEnding)}`"></nuxt-link>
            ...
            <div class="flex flex-row items-center gap-2 sm:contents">
              <div class="self-center">
                <div class="relative z-[1] cursor-pointer">
                  <VersionChannelIndicator v-tooltip="`Toggle filter for ${version.version_type}`"
                    :channel="version.version_type"
                    @click="versionFilters.toggleFilter('channel', version.version_type)" />
                </div>
              </div>
              <div class="pointer-events-none relative z-[1] flex flex-col justify-center group-hover:underline">
            ...
            <div class="flex flex-col justify-center gap-2 sm:contents">
              <div class="flex flex-row flex-wrap items-center gap-1 xl:contents">
                <div class="flex items-center">
                  <div class="tag-list">
                    <div v-for="gameVersion in formatVersionsForDisplay(version.game_versions)"
                      :key="`version-tag-${gameVersion}`" v-tooltip="`Toggle filter for ${gameVersion}`"
                ...
                <div class="flex items-center">
                  <div class="tag-list">
                    <div v-for="platform in version.loaders" :key="`platform-tag-${platform}`"
              ...
              <div
                ...
                <div class="pointer-events-none z-[1] flex items-center gap-1 font-medium xl:self-center">
          ...
          <div class="flex items-start justify-end gap-1 sm:items-center">
            <ButtonStyled circular type="transparent">
              <a v-if="!version.disk_only" v-tooltip="`下载`" :href="getPrimaryFile(version).url"
                class="z-[1] group-hover:!bg-brand group-hover:!text-brand-inverted" aria-label="Download"
                @click="emits('onDownload')">
                <DownloadIcon aria-hidden="true" />
              </a>
              <a v-else v-tooltip="`下载`" target="_blank" :href="getPrimaryDiskUrl(version)"
                class="z-[1] group-hover:!bg-brand group-hover:!text-brand-inverted" aria-label="Download"
            ...
            <ButtonStyled circular type="transparent">
              <OverflowMenu class="group-hover:!bg-button-bg" :options="[
                // {
                //   id: 'download',
                //   color: 'primary',
                //   hoverFilled: true,
                ...
                },
                {
                  ...
                  action: () =>
                    copyToClipboard(
                      `https://bbsmc.net/${project.project_type}/${project.slug ? project.slug : project.id
                      }/version/${encodeURI(version.displayUrlEnding)}`,
                    ),
                },
                {
          ...
          <div v-if="flags.showVersionFilesInTable" class="tag-list pointer-events-none relative z-[1] col-span-full">
            <div v-for="(file, fileIdx) in version.files" :key="`platform-tag-${fileIdx}`"
    ...
    <div class="my-3 flex justify-end">
      <Pagination :page="currentPage" :count="Math.ceil(filteredVersions.length / 20)"
        :link-function="(page) => `?page=${currentPage}`" @switch-page="switchPage" />
    </div>
  </section>
</template>
