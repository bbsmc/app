...
mod tests {
    ...
    #[test]
    fn test_valid_paths() {
        assert!(SafeRelativePath::new("mods/example.jar".to_string()).is_ok());
        assert!(SafeRelativePath::new("config/mod.json".to_string()).is_ok());
        assert!(SafeRelativePath::new("file.txt".to_string()).is_ok());
    }

    #[test]
    fn test_invalid_paths() {
        // 空路径
        assert!(SafeRelativePath::new("".to_string()).is_err());

        // 反斜杠
        assert!(
            SafeRelativePath::new("mods\\example.jar".to_string()).is_err()
        );

        // 路径遍历
        assert!(SafeRelativePath::new("../etc/passwd".to_string()).is_err());
        assert!(
            SafeRelativePath::new("mods/../../../etc/passwd".to_string())
                .is_err()
        );
        ...
        // Windows 保留名称
        assert!(SafeRelativePath::new("CON".to_string()).is_err());
        assert!(SafeRelativePath::new("mods/NUL.jar".to_string()).is_err());
        assert!(SafeRelativePath::new("COM1".to_string()).is_err());
    }
}
